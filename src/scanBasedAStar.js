...
export class ScanBasedPathfinder extends PathFinder {
	/**
	 * @param {{
	 * 	mapScene: THREE.Scene,
	 * 	scanRadius: number,
	 ...
	 * }} param0
	 */
	constructor({ mapScene, scanRadius, stepLength = scanRadius / 2 }) {
		super();
		this.scanHelper = new ScanHelper({ mapScene, scanRadius });
		this.stepLength = stepLength;
		/** @type {AStarContext|null} */
		this.warmupContext = null;
	}
	/**
	 * @param {THREE.Vector3} startPos
	...
	async *warmUp(startPos) {
		...
		this.warmupContext = new AStarContext(startPos, null, this.stepLength, this.scanHelper);
		this.warmupContext.addEventListener("reachableNodeFound", (event) => {
			this.dispatchEvent(/** @type {ReachableNodeFoundEvent} */({ type: "reachableNodeFound", node: event.node, source: "forward" }));
		});
		for await (const _iteratorResult of this.warmupContext) {
	...
	async findPath(startPos, endPos) {
		if (ASTAR_DEBUG_LOG) console.info("Launching A*", startPos, endPos);
		Object.freeze(startPos);
		Object.freeze(endPos);
		if (ASTAR_DEBUG_LOG) console.time("A*");
		if (!this.warmupContext) this.warmupContext = new AStarContext(startPos, endPos, this.stepLength, this.scanHelper, true);
		this.warmupContext.endPos = endPos;
		const aStarContext = new BiDirectionalAStarContext(this.warmupContext);
		aStarContext.addEventListener("reachableNodeFound", ({ node, context } = /** @type {any} */({})) => {
			const source = context instanceof BackwardAStarContext ? "backward" : "forward";
			this.dispatchEvent(/** @type {ReachableNodeFoundEvent} */({ type: "reachableNodeFound", node, source }));
		});
		let stepCount = 0;
		do {
			await aStarContext.next();
			stepCount++;
			if (stepCount % 5 == 0) await new Promise(requestAnimationFrame);
		} while (!aStarContext.ended);
		if (ASTAR_DEBUG_LOG) console.timeEnd("A*");
		this.warmupContext = new AStarContext(startPos, null, this.stepLength, this.scanHelper);
		const result = aStarContext.result?.map(v => v.position) ?? null;
		if (ASTAR_DEBUG_LOG && result) console.info("Result position list", result);
		return result;
	}
}
